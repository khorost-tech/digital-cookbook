...
| [`architecture/resilience`](architecture/resilience) | Backpressure и load shedding: bounded-очередь, AIMD-лимитер | [статья](https://khorost.tech/architecture/backpressure-load-shedding/) |
...
| Пример | Описание | Статья |
|---|---|---|
| [`messaging/data-streaming`](messaging/data-streaming) | Сквозной пайплайн: PostgreSQL → Debezium/Kafka Connect → Kafka → Kafka Streams → ClickHouse | [статья](https://khorost.tech/messaging/streaming-pipeline-cdc-to-clickhouse/) |
...
| [`go/generics`](go/generics) | Дженерики Go на бенчмарках: generic vs interface{} vs рукопись, dispatch, размер бинаря | [статья](https://khorost.tech/go/go-generics/) |
