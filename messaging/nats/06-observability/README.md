...
`server report` и `nats top` ходят через системные (`$SYS`) subjects, а `$SYS` — особый внутренний account: даже когда аутентификация не включена, обычный клиент попадает в дефолтный account (`$G`) и доступа к `$SYS` не имеет. Поэтому в этом демо **`server report`/`nats top` возвращают ошибку авторизации** — им нужен явный `$SYS`-доступ (system-пользователь/контекст, ст. 8 серии), который здесь не заведён. Разовую проверку JetStream без прав делайте через `nats server check jetstream` (работает без `$SYS`) или через HTTP monitoring endpoints на `:8222`.
