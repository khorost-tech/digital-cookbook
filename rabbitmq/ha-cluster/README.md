...
| Нода | AMQP | Management UI | Prometheus | Роль |
|------|------|---------------|------------|------|
| rabbit1 | 5672 | 15672 | 15692 | Кластер A (upstream для federation) |
| rabbit2 | 5673 | 15673 | — | Кластер A |
| rabbit3 | 5674 | 15674 | — | Кластер A |
| rabbit-b | 5681 | 15681 | — | Кластер B (downstream для federation) |
| rabbit-legacy (3.13) | 5680 | 15680 | — | Legacy demo |
...
```bash
AUTO=1 bash scripts/federation-demo.sh
```

**Топология (два независимых кластера):**
...
**Ключевые свойства:**
- `rabbit-b` — **отдельный кластер** с другим Erlang cookie (`CLUSTERB_COOKIE_XYZ`). Не вступает в кластер rabbit1/2/3.
- Оба кластера находятся в одной docker-сети `rabbitnet`, что позволяет rabbit-b подключаться к rabbit1 по AMQP.
- Federation — **однонаправленная**: сообщения, опубликованные в exchange `demo.fed` на rabbit1, доставляются в очередь `demo.fed.q` на rabbit-b.
- **URI важен:** vhost `/` должен быть url-encoded как `%2F` в AMQP URI. Значение `amqp://...@rabbit1:5672/` (trailing slash) в RabbitMQ 4.x означает пустой vhost → ошибка `not_allowed`.

**Что происходит в демо:**
1. Exchange `demo.fed` объявляется на обоих кластерах.
2. На rabbit-b создаётся очередь `demo.fed.q`, привязанная к `demo.fed`.
3. На rabbit-b задаётся federation-upstream (`upstream-rabbit1` → `amqp://demo:demo@rabbit1:5672/%2F`).
4. На rabbit-b применяется policy `federate-fed-b` → federation-link устанавливается за ~5 сек.
5. Сообщение публикуется в rabbit1 (upstream) — и **реально** появляется в `demo.fed.q` на rabbit-b (downstream).

**Management UI:**
- UPSTREAM rabbit1: http://localhost:15672 (demo/demo)
- DOWNSTREAM rabbit-b: http://localhost:15681 (demo/demo)

**Статус federation-links** — на вкладке Admin → Federation Status в http://localhost:15681 или:

```bash
# Статус linków на downstream (rabbit-b)
docker exec rabbit-b rabbitmqctl list_parameters
docker exec rabbit-b rabbitmqctl eval 'rabbit_federation_status:status().'

# Очереди на downstream после публикации в upstream
docker exec rabbit-b rabbitmqctl list_queues name messages
```

**Очистка артефактов федерации:**
