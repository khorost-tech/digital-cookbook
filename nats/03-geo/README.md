...
`$SYS`-аккаунт (`accounts{SYS:{users:[{user:sys,password:sysdemo}]}}`) настроен на всех трёх JetStream-нодах — это обязательное условие nats-server, когда на ноде одновременно включены `gateway{}` и `leafnodes{}` (или JetStream-кластер поверх gateway). `$G` при этом остаётся без аутентификации: SYS — единственный явно заданный аккаунт, поэтому implicit `no_auth_user` для `$G` не отключается (см. [GHSA-fr2g-9hjm-wr23](https://github.com/nats-io/nats-server/security/advisories/GHSA-fr2g-9hjm-wr23), исправлено в 2.10.2+, у нас 2.12 — безопасно). Обычные `nats pub`/`sub`/`stream` команды не требуют логина.
