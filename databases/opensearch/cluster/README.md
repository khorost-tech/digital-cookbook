...
> прогоном на OpenSearch 3.5.0; у OpenSearch нет единого «одинакового на всех инсталляциях»
...
```
opensearch/cluster/
├── deploy.sh                           # Точка входа: клонирует upstream-плейбук, затем deploy.yml
├── deploy.yml                          # Конфигурация поверх upstream: import_playbook,
│                                        # TLS (self-signed), securityconfig
├── inventory/
...
`deploy.sh` клонирует (или обновляет) upstream-плейбук в `.upstream/`, затем запускает
`ansible-playbook -i inventory/hosts.ini deploy.yml`. Клонирование вынесено в скрипт не для
удобства, а по необходимости: `import_playbook` в `deploy.yml` — статический импорт, Ansible
резолвит путь к upstream `opensearch.yml` на этапе парсинга (до запуска задач), поэтому
upstream должен существовать **до** старта `ansible-playbook`, а не клонироваться внутри того
же прогона.

По шагам:

1. `deploy.sh` клонирует `opensearch-project/ansible-playbook` в `.upstream/` (или обновляет);
2. `deploy.yml` импортирует upstream `opensearch.yml` — Ansible на весь прогон автоматически
   подхватывает наш `group_vars/all.yml` (`os_version`, `xms_value`/`xmx_value`) и
   `inventory/hosts.ini` (`ip=`/`roles=` на каждый хост — из них upstream-плейбук сам выводит
   discovery.seed_hosts и cluster.initial_cluster_manager_nodes); upstream-репозиторий при этом
   не модифицируется. Если вашей топологии нужны переменные, которых нет в нашем
   `group_vars/all.yml`, — посмотрите дефолты upstream в
   `.upstream/ansible-playbook/inventories/opensearch/group_vars/all/all.yml`
...
`--syntax-check` тоже требует, чтобы upstream уже был склонирован (тот же статический импорт),
поэтому сначала клонируйте его — либо один раз запустив `./deploy.sh`, либо вручную:

```bash
git clone --depth 1 https://github.com/opensearch-project/ansible-playbook .upstream/ansible-playbook
