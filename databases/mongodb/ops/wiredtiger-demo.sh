#!/usr/bin/env bash
# wiredtiger-demo.sh — оркестрирует стенд #2 серии "MongoDB: глубокое
# погружение" (wiredtiger/): движок хранения WiredTiger вживую —
# cache/checkpoint/journal — на РЕАЛЬНОМ 3-узловом replica set (rs0). Без
# PostgreSQL (см. wiredtiger/main.go — контраст с PG WAL остаётся
# граничной ссылкой в тексте статьи, не отдельным измерением на этом
# стенде).
#
# Шаги (тот же приём, что и ops/modeling-demo.sh):
...
#      стенду нужен реалистичный объём данных (полный датасет Task 3), а не
#      пустой кластер: рост dirty bytes/checkpoint/журнала сравнивается
#      против уже ненулевого фонового состояния кеша после импорта.
#   5. Запустить wiredtiger/main.go в golang:1.25 НА СЕТИ
#      mongodb-cookbook_default (--network) — стенд подключается к
#      mongo1/mongo2/mongo3 по DNS-именам сервисов, снимает
#      serverStatus().wiredTiger до/после read- и write-нагрузки и после
#      принудительного checkpoint (fsync:1), печатает FIXTURE-строки и
#      падает (log.Fatalf, ненулевой exit) при провале ассерта.
#   6. down -v (стенд эфемерный — не остаётся висеть между прогонами).
#
# Запуск (из mongodb/):
#   bash ops/wiredtiger-demo.sh 2>&1 | tee /tmp/mongo-wiredtiger.txt
# Требует: docker. Поднимает 3 контейнера (mongo1/2/3), сеть, запускает 2
# одноразовых контейнера golang:1.25 (dataset gen + стенд).

set -euo pipefail
# На Git Bash/MSYS docker-аргументы вида -w /app/wiredtiger иначе
# мангаются в windows-путь; на Linux/WSL переменная безвредна (см. тот же
# приём в ops/verify-static.sh и ops/modeling-demo.sh).
...
step "5/6 стенд wiredtiger/main.go (golang:1.25, сеть $NETWORK) — cache/checkpoint/journal вживую"
set +e
docker run --rm --network "$NETWORK" \
  -e MONGO_URI="$MONGO_URI" \
  -v "$ROOT:/app" -v "$ROOT/.gocache:/go/pkg/mod" -w /app/wiredtiger golang:1.25 \
  go run .
STAND_EXIT=$?
set -e
...
if [ "$STAND_EXIT" -ne 0 ]; then
  echo "wiredtiger: стенд завершился с ошибкой (exit=$STAND_EXIT) — см. лог выше (assert упал или сбой соединения)." >&2
  exit "$STAND_EXIT"
fi

echo
echo "wiredtiger-demo: готово. FIXTURE-строки выше -> дословно в FIXTURES.md §2."
