...
// паттерн, что ../materialized-views/csvrow.go / ../go/mergetree/csvrow.go —
// код продублирован умышленно (свой независимый Go-модуль стенда, без
// общего internal-пакета на этом этапе серии). revenue сканируется в
// string, не decimal.Decimal — этот стенд не оперирует суммами (только
// count()), лишняя зависимость shopspring/decimal не нужна.
type row struct {
	eventTime  time.Time
	userID     uint64
	eventType  string
