...
// row — типизированная строка общего датасета (../dataset/main.go), без
// суррогатного id (не нужен для этого стенда — см. schema.go).
type row struct {
	eventTime  time.Time
	userID     uint64
	eventType  string
...
// что в ../../when-olap/load.go (отдельный модуль — код продублирован
// умышленно: два независимых Go-модуля стендов, без общего internal-пакета
// на этом этапе серии).
