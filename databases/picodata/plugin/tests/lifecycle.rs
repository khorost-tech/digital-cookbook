...
fn test_smoke_plugin_lifecycle() {
    ...
    // 1) CREATE PLUGIN
    cluster
        .run_query(format!(r#"CREATE PLUGIN "{plugin_name}" {version};"#))
        .unwrap_or_else(|err| panic!("CREATE PLUGIN failed: {err}"));

    // 2) ADD SERVICE TO TIER — обязательный шаг: без него сервис не привязан
    // ни к одной группе инстансов. Полный порядок жизненного цикла:
    // CREATE -> ADD SERVICE -> MIGRATE -> ENABLE.
    cluster
        .run_query(format!(
            r#"ALTER PLUGIN "{plugin_name}" {version} ADD SERVICE "near_data_service" TO TIER "default";"#
        ))
        .unwrap_or_else(|err| panic!("ADD SERVICE TO TIER failed: {err}"));
    ...
    // 4) ENABLE
    cluster
        .run_query(format!(r#"ALTER PLUGIN "{plugin_name}" {version} ENABLE;"#))
