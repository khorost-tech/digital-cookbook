...
// multihostTimeout — отдельный, заведомо больший бюджет для стартовой проверки по
// MULTIHOST_DSN. Секунды opTimeout хватает только пока все хосты из DSN живы: после
// падения primary его имя исчезает из встроенного DNS Docker, резолв уходит на
// upstream и висит секундами, съедая весь бюджет ещё до попытки второго хоста —
// то есть ровно в момент failover демонстрация и переставала работать
// ("hostname resolving error ... i/o timeout" на обоих хостах). Перебор N хостов
// стоит N резолвов плюс N попыток коннекта — это не операция уровня одного запроса.
const multihostTimeout = 15 * time.Second
...
func logMultihostSelection(ctx context.Context) {
	...
	cctx, cancel := context.WithTimeout(ctx, multihostTimeout)
	defer cancel()
	...
	// Хост спрашиваем у САМОГО сервера (как в Java-клиенте), а не из conn.Config():
	// у pgx Config.Host/Port — это первый хост, разобранный из DSN, а не тот, на котором
	// перебор в итоге остановился. После промоушена реплики соединение честно уезжает
	// на неё, но conn.Config() всё равно вернул бы pg-primary — то есть демонстрация
	// показывала бы старый хост ровно в том сценарии, ради которого она написана.
	var addr string
	var port int
	var inRecovery bool
	if err := conn.QueryRow(cctx,
		"SELECT inet_server_addr()::text, inet_server_port(), pg_is_in_recovery()",
	).Scan(&addr, &port, &inRecovery); err != nil {
		fmt.Printf("[go] ERROR multihost probe: %v\n", err)
		return
	}
	fmt.Printf("[go] MULTIHOST: target_session_attrs=read-write -> выбран хост %s:%d (in_recovery=%t)\n",
		addr, port, inRecovery)
}
