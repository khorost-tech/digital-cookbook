...
async fn build_writer_pool() -> Pool<Postgres> {
    ...
    // connect_lazy_with, а не connect_with: пул создаётся без немедленного соединения.
    // Eager-вариант падал бы в fatal() ещё до цикла, если primary/bouncer сейчас недоступны —
    // ровно в сценарии рестарта клиента после failover, — вместо заявленного в шапке файла
    // "логируем ошибку и продолжаем". Ошибки соединения при этом не теряются: они приходят
    // из acquire() на каждой итерации, где уже есть обработка и ретраи.
    PgPoolOptions::new()
        .max_connections(10)
...
async fn build_reader_pool() -> Pool<Postgres> {
    ...
    // connect_lazy — см. комментарий в build_writer_pool: пул не должен падать, если реплика
    // сейчас недоступна. Result здесь только про разбор DSN, а не про сетевое соединение.
    PgPoolOptions::new()
        .max_connections(10)
        .max_lifetime(Duration::from_secs(5 * 60))
        .acquire_timeout(OP_TIMEOUT)
        .connect_lazy(&dsn)
        .unwrap_or_else(|e| fatal(&format!("не удалось разобрать REPLICA_DSN: {e}")))
}
...
/// транзиентных ошибок.
///
/// Тип id — i32, а не i64: колонка объявлена как `serial` (то есть INT4), а sqlx проверяет
/// соответствие типов строго и отказывается расширять INT4 до i64 ("mismatched types;
/// Rust type `i64` (as SQL type `INT8`) is not compatible with SQL type `INT4`"). pgx (Go)
/// и pgjdbc (Java) такое расширение делают молча — это заметное различие в строгости
/// клиентов на одной и той же схеме.
async fn write_row(pool: &Pool<Postgres>) -> Result<i32, sqlx::Error> {
    let name = format!("client-rust@{}", now_hms_millis());

    with_retry(|| {
        let name = name.clone();
        async move {
            let fut = sqlx::query_scalar::<_, i32>("INSERT INTO users (name) VALUES ($1) RETURNING id")
                .bind(name)
                .fetch_one(pool);
