...
// runScylla: LWT-резервация. BATCH в Scylla/Cassandra — это НЕ транзакция (атомарность
// применения записей, не изоляция). Настоящая линеаризуемость — только через LWT (условие
// IF [NOT EXISTS]) на Paxos, и только в пределах одного раздела, ценой раундов консенсуса.
