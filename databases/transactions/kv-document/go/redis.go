...
//   - naive GET + SET: между двумя командами разных клиентов нет изоляции → lost update.
//   - WATCH + MULTI/EXEC (оптимистичный CAS) с retry: EXEC не применяется, если ключ изменился
//     после WATCH; повторяем — итог корректен. Это единственная «транзакционность» Redis:
//     не изоляция и не rollback, а оптимистичная проверка версии ключа.
func runRedis() {
