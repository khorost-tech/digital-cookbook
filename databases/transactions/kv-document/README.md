...
|---------|----------|----------------|
| Redis | `WATCH` + `MULTI/EXEC` (оптимистичный CAS) | `MULTI/EXEC` исполняется атомарно и без чужих вклиниваний, но rollback нет; `WATCH` добавляет условный CAS — отменяет `EXEC`, если ключ изменился. Гонку даёт не транзакция, а naive GET/SET с чтением вне неё |
